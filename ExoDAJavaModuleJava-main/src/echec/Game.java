...
public class Game {
	...
	public Game()
	{
		...
		System.out.println("Entrez vote nom de joueur 1");
		w = entree.nextLine();
		...
		Joueur j1=new Joueur (w,Couleur.WHITE);
		Joueur j2=new Joueur (x,Couleur.BLACK);
	...
	private void update()
	{
		...
		for (Roi count:Roi.tabRoi)
		{
			...
			if(count.isEchec() && !count.echecEtMat)
				System.out.println("Le roi "+count.getCouleur()+ " est en echec");
			else if (count.isEchec() && count.echecEtMat || count.isEchecEtPat())
			{
				over=true;
